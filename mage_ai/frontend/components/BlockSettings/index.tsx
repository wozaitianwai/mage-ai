...
type BlockSettingsProps = {
  ...
  showUpdateBlockModal?: (block: BlockType, name: string, isReplacingBlock?: boolean) => void;
...
function BlockSettings({
  ...
  const pipelineRetryConfig: PipelineRetryConfigType = useMemo(
  ...
  }, [configuration, globalDataProductsByUUID]);
  ...
  const blockRetryConfig: BlockRetryConfigType = useMemo(
    () => blockAttributes?.retry_config || {},
    [blockAttributes],
  );
  ...
  }, [editCustomExecutorType, executorType]);
  ...
  const blockPipelines: BlockPipelineType[] = useMemo(
    () => (blockDetails?.pipelines ? blockDetails?.pipelines : []),
    [blockDetails],
  );
        ...
        <TableContainerStyle>
          <Table
            ...
            rows={blockPipelines.map(
              ({ pipeline: { name: pipelineName, repo_path: repoPath, uuid: pipelineUUID } }) => {
                let nameEl;
  ...
  const objectAttributes = useMemo(
    () => blockAttributes?.configuration?.global_data_product || {},
    [blockAttributes],
  );
  ...
  const isDataIntegration = useMemo(
    () => isDataIntegrationBlock(block, pipeline),
    [block, pipeline],
  );
  const streams = useMemo(
    () => isDataIntegration && getSelectedStreams(block),
    [block, isDataIntegration],
  );
    ...
    <>
      <Spacing mb={UNITS_BETWEEN_SECTIONS} pt={PADDING_UNITS}>
        ...
        <Spacing mb={UNITS_BETWEEN_SECTIONS} px={PADDING_UNITS}>
          <RowDataTable noBackground noBoxShadow sameColorBorders>
            <RowStyle noBorder>
              <FlexContainer {...JUSTIFY_SPACE_BETWEEN_PROPS}>
                <Flex>
                    <Text bold default>
              ...
              <RowStyle noBorder>
                <FlexContainer {...JUSTIFY_SPACE_BETWEEN_PROPS}>
                  ...
                  <Button
                    ...
                    afterIcon={
                      <Circle
                        color={blockColor ? BLOCK_COLOR_HEX_CODE_MAPPING[blockColor] : null}
            ...
            <Checkbox
              ...
              onClick={() =>
                setBlockAttributes((prev) => ({
                  ...
                  configuration: {
                    ...prev?.configuration,
                    disable_output_preview: !prev?.configuration?.disable_output_preview,
                  },
                }))
              }
            />
        ...
        */}
        ...
        {BLOCK_TYPES_WITH_VARIABLES.includes(blockType) &&
